"""
PixelMind VLM Supervised Fine-Tuning
=====================================
Stage 4: Fine-tune the VLM on multi-turn visual question-answering data.

Trains the projector + LLM first/last layers; vision encoder is frozen.

Usage:
    python -m pixelmind.trainer.vlm_sft --from_weight pretrain_vlm --data_path ../data/multimodal/sft_i2t.parquet

Recommended: lr=5e-6, epochs=1, batch=4, max_seq=768, freeze_llm=1
"""
...
def main():
    parser = argparse.ArgumentParser(description="PixelMind VLM SFT")
    ...
    parser.add_argument("--data_path", type=str, default="../data/multimodal/sft_i2t.parquet")
    parser.add_argument("--from_weight", default="pretrain_vlm", type=str, help="VLM base weight")
    ...
    parser.add_argument("--wandb_project", type=str, default="PixelMind-VLM-SFT")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb.init(
            ...
            name=f"PixelMind-VLM-SFT-E{args.epochs}-B{args.batch_size}-LR{args.learning_rate}",
            id=wandb_id,
            resume=resume,
        )
